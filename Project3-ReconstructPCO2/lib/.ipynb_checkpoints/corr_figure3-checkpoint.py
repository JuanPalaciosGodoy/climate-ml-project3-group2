...
def decompose_stl_fast_parallel(da, var_name, prefix='spco2', lo_frac=0.1, lo_delta=0.01, it=3, period=12, n_jobs=-1):
    data = xr.open_zarr(da, consolidated=True)
    ds_out = decompose_stl_fast_parallel_from_xarray(
        data=data,
        var_name=var_name,
        prefix=prefix,
        lo_frac=lo_frac,
...
def decompose_stl_fast_parallel_from_xarray(data, var_name, prefix='spco2', lo_frac=0.1, lo_delta=0.01, it=3, period=12, n_jobs=-1) -> xr.Dataset:
    data = data.dropna(dim="time", how="all")
    data_detrend = detrend_fast_parallel(data[var_name], dim='time', n_jobs=n_jobs)
    data_trend = data[var_name] - data_detrend
    ...
    ds_out = xr.Dataset(
        {
            ...
            f'{prefix}_residual_low': data_residual_low,
        },
